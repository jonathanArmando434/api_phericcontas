...
const getMonthlyPerformance = (tarefa) => {
  ...
  for (let i = 0; i < 12; i++)
    if (qntMonthlyFinished[i] > 0)
      monthlyPerformance[i] =
        (qntMonthlyOnTime[i] / qntMonthlyFinished[i]) * 100;
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Tarefa inserida no sistema com sucesso!" });
  } catch (error) {
    console.log(error);
    res
      .status(500)
...
exports.findMany = async (req, res) => {
  ...
  try {
    let tarefa =
      (await Tarefa.find({ id_cliente: id }).sort({ criado_em: -1 })) || {};

    if (Object.keys(tarefa).length === 0) {
      tarefa =
        (await Tarefa.find({ id_responsavel: id }).sort({ criado_em: -1 })) ||
        {};
      if (Object.keys(tarefa).length === 0) {
...
exports.annualReport = async (req, res) => {
  ...
  try {
    ...
    const percentFinished =
      getPercentAboutOneYearAgo(qntFinished, oneYearAgo.qntFinished) || 0;

    const qntCanceled = getQntStatus(tarefa, "Cancelado");
    const percentCanceled =
      getPercentAboutOneYearAgo(qntCanceled, oneYearAgo.qntCanceled) || 0;

    const qntInProgress = getQntStatus(tarefa, "Em progresso");
    const percentInProgress =
      getPercentAboutOneYearAgo(qntInProgress, oneYearAgo.qntInProgress) || 0;

    const qntOne = getQntService(tarefa, "Organização contabilística");

    const qntTwo = getQntService(
      tarefa,
...
exports.annualReportAboutAssociate = async (req, res) => {
  ...
  try {
    ...
    const qntFinishedOnTime = getQntFinishedOnTime(tarefa);
    const percentFinishedOnTime =
      getPercentAboutOneYearAgo(
        qntFinishedOnTime,
        oneYearAgo.qntFinishedOnTime
      ) || 0;

    const qntFinishedWithDelay = getQntFinishedWithDelay(tarefa);
    const percentFinishedWithDelay =
      getPercentAboutOneYearAgo(
        qntFinishedWithDelay,
        oneYearAgo.qntFinishedWithDelay
      ) || 0;
    ...
    res.status(200).json({
      finishedOnTime: {
        qnt: qntFinishedOnTime,
        percent: percentFinishedOnTime,
      },
      finishedWithDelay: {
        qnt: qntFinishedWithDelay,
        percent: percentFinishedWithDelay,
      },
      total: { qnt: total, percent: percentTotal },
      monthlyPerformance,
    });
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    let auxNewTarefa = {};
    if (!data_fim) {
      const { data_fim, ...otherAux } = newTarefa;
      auxNewTarefa = { ...otherAux };
    }
    if (!status) {
      const { status, ...otherAux } = auxNewTarefa;
      auxNewTarefa = { ...otherAux };
    } else auxNewTarefa = { ...newTarefa };

    await Tarefa.updateOne({ _id: id }, auxNewTarefa);
...
exports.remove = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Houve um erro no sistema, tente novamente!" });
  }
};
